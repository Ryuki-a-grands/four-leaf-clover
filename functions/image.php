...
function four_leaf_clover_image_create( $change_r = 192, $change_g = 224, $change_b = 181 ) {
	if ( four_leaf_clover_gd_library_active() ) {
		$im = imagecreatefromjpeg( get_template_directory_uri() . '/images/default/four-leaf-clover.jpg' );
		$search = imagecreatefromjpeg( get_template_directory_uri() . '/images/default/search.jpg' );
		imagefilter( $im,IMG_FILTER_COLORIZE,$change_r,$change_g,$change_b,0 );
...
function four_leaf_clover_hex2rgb( $hex ) {
	$hex = str_replace( '#', '', $hex );
	if ( strlen( $hex ) == 3 ) {
