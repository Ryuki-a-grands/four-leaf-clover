...
function four_leaf_clover_setup() {
	...
	register_nav_menu( 'header-navi', esc_html__( 'Primary Navigation','four-leaf-clover' ) );
...
if ( ! function_exists( 'four_leaf_clover_adds_footer' ) ) {
	function four_leaf_clover_adds_footer() {
		...
		foreach ( $users as $user ) {
			if ( $user->ID === $id ) {
				$copyright_owner_name = $user->display_name;
				break;
			}
		}
		?>
	...
	?>
			<div class="left">Copyright &copy;<?php echo esc_html( $year );?> <a href="<?php echo esc_html( home_url( '/' ) );?>"><?php echo esc_attr( $copyright_owner_name );?></a> All Rights Reserved.</div>
<?php
	endif;
		?>
			<div class="right"><?php esc_html_e( 'Use theme of four-leaf clover', 'four-leaf-clover' ); ?><br/>
			<a href="<?php esc_html_e( 'https://wordpress.org/', 'four-leaf-clover' ); ?>"><?php esc_html_e( 'Proudly powered by WordPress', 'four-leaf-clover' ); ?></a></div>
		</footer>
	<?php
	}
}// End if().
